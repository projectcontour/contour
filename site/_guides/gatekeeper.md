...
This framework has two key concepts: `ConstraintTemplate` and `Constraint`.
...
Now, let's create some `HTTPProxies` to see the validations in action.
...
Now, let's create an `HTTPProxy` to see the validation in action.
...
You should see something like:
```
Error from server ([denied by httpproxy-retry-count-range] retry policy count must be less than/equal to 5): error when creating "proxy-retries.yml": admission webhook "validation.gatekeeper.sh" denied the request: [denied by httpproxy-retry-count-range] retry policy count must be less than/equal to 5
```
...
Status:
    ...
    Violations:
        ...
        Kind:                HTTPProxy
        Message:             retry policy count must be less than/equal to 4
        Name:                demo-retries
        Namespace:           default
```
...
If you're using Gatekeeper, we strongly recommend that you configure all of the **validations** we've defined, since these rules are already being checked internally by Contour and reported as status errors/invalid proxies.
...
The **validations** can be found in `examples/gatekeeper/02-validation-constraint-templates.yml` and `examples/gatekeeper/03-validation-constraints.yml`.
...
The sample **policies** can be found in `examples/gatekeeper/04-policy-constraint-templates.yml` and `examples/gatekeeper/05-policy-constraints.yml`.
