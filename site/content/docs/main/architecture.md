...
It has built-in leader election which is responsible for updating httproxy/ingress/gateway api resources via Kube APi server. So in this case even if we have multiple replicas only one of them is acting at a time. All instances are able to serve xDS configuration to any Envoy instance, but only the leader can write status back to the API server.
