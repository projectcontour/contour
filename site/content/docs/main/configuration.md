...
| accesslog-level           | string                 | `info`                                                                                               | This field specifies the verbosity level of the access log. Valid options are `info` (default, all requests are logged), `error` (all non-success requests are logged), `critical` (all 5xx requests are logged) and `disabled`. |
