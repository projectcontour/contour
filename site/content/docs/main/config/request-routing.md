...
3. two optional modifiers: `ignoreCase` and `treatMissingHeadersAsEmpty`
...
- `treatMissingHeadersAsEmpty`: specifies if the header match rule specified header does not exist, this header value will be treated as empty. Defaults to false. Unlike the underlying Envoy implementation this is **only** supported for negative matches (e.g. NotContains, NotExact).
