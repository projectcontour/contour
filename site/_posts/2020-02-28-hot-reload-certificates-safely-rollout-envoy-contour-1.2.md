...
Contour continues to add new features to help manage how it operates in a cluster. Our latest release, Contour 1.2, now includes support for certificate rotation for xDS gRPC interface between Contour and Envoy. Additionally, Contour 1.2 includes a new subcommand which assists in Envoy rollouts in your cluster to minimize the number of connection errors.

## Hot-Reload Certificates

A few releases ago, Contour enabled secure communication between Contour and Envoy. This new feature ensured that any communication between Contour and Envoy over its gRPC connection would be secure, mainly securing the TLS certificate keys.
...
Future steps are to also enable this same functionality for Envoy which has some [open issues to be solved in Envoy first](https://github.com/envoyproxy/envoy/issues/9359).
...
As with any application rollout strategy, we want a way to implement the rollout while minimizing the effects on users. If the Envoy pods are terminated while there are still open connections, then users will receive errors.
...
![Envoy Shutdown Manager](/img/posts/contour-1.2/envoy-shutdown-manager.png)
