...
// +kubebuilder:validation:XValidation:rule="self.count != -1 || (!has(self.perTryTimeout) && size(self.retryOn) == 0 && size(self.retriableStatusCodes) == 0)",message="perTryTimeout, retryOn, and retriableStatusCodes must not be set when retries are disabled"
