...
type VirtualHost struct {
	...
	// IncludeRequestAttemptCount, when true, causes Envoy to include the
	// x-envoy-attempt-count header in requests forwarded to the upstream.
	// The value starts at 1 for the initial attempt and is incremented for
	// each retry, allowing upstream services to observe how many times a
	// request has been attempted.
	// +optional
	IncludeRequestAttemptCount bool `json:"includeRequestAttemptCount,omitempty"`

	// IncludeAttemptCountInResponse, when true, causes Envoy to include the
	// x-envoy-attempt-count header in responses returned to the downstream
	// client, reflecting the number of times the request was attempted on the
	// upstream.
	// +optional
	IncludeAttemptCountInResponse bool `json:"includeAttemptCountInResponse,omitempty"`
}
