...
func TestConvertServeContext(t *testing.T) {
	cases := map[string]struct {
		...
		"tls ecdh_curves": {
			getServeContext: func(ctx *serveContext) *serveContext {
				ctx.Config.TLS.ECDHCurves = []string{"X25519", "P-256", "P-521", "X25519MLKEM768", "MLKEM1024"}
				return ctx
			},
			getContourConfiguration: func(cfg contour_v1alpha1.ContourConfigurationSpec) contour_v1alpha1.ContourConfigurationSpec {
				cfg.Envoy.Listener.TLS.ECDHCurves = []string{"X25519", "P-256", "P-521", "X25519MLKEM768", "MLKEM1024"}
				return cfg
			},
		},
		"tls fingerprinting": {
