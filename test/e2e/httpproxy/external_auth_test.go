...
func testExternalAuth(namespace string) {
	Specify("external auth can be configured on an HTTPProxy", func() {
		...
		By("FailOpen=false, unreachable auth server returns 403")
		f.Certs.CreateSelfSignedCert(namespace, "failopen-false", "failopen-false.externalauth.projectcontour.io")
		failOpenFalseProxy := &contour_v1.HTTPProxy{
			...
			Spec: contour_v1.HTTPProxySpec{
				VirtualHost: &contour_v1.VirtualHost{
					Fqdn: "failopen-false.externalauth.projectcontour.io",
					TLS: &contour_v1.TLS{
						SecretName: "failopen-false",
					},
					Authorization: &contour_v1.AuthorizationServer{
						FailOpen:        false,
						ResponseTimeout: "500ms",
