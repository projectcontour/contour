...
func TestListenerVisit(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"httpproxy with HTTPBufferMaxRequestBytes set in listener config": {
			ListenerConfig: ListenerConfig{
				HTTPBufferMaxRequestBytes: 10,
			},
			objs: []any{
				&contour_v1.HTTPProxy{
					ObjectMeta: meta_v1.ObjectMeta{
			...
			want: listenermap(&envoy_config_listener_v3.Listener{
				...
				FilterChains: envoy_v3.FilterChains(
					envoy_v3.HTTPConnectionManagerBuilder().
						...
						AccessLoggers(envoy_v3.FileAccessLogEnvoy(DEFAULT_HTTP_ACCESS_LOG, "", nil, contour_v1alpha1.LogLevelInfo)).
						DefaultFilters().
						AddFilter(envoy_v3.BufferFilter(10)).
						Get(),
				),
				SocketOptions: envoy_v3.NewSocketOptions().TCPKeepalive().Build(),
			}),
		...
		"httpsproxy with HTTPSBufferMaxRequestBytes set in listener config": {
			ListenerConfig: ListenerConfig{
				HTTPSBufferMaxRequestBytes: 10,
			},
			objs: []any{
				&contour_v1.HTTPProxy{
					ObjectMeta: meta_v1.ObjectMeta{
		...
		"httpproxy with HTTPBufferMaxRequestBytes equals 0 in listener config": {
			ListenerConfig: ListenerConfig{
		...
		"httpsproxy with HTTPSBufferMaxRequestBytes equals 0 in listener config": {
			ListenerConfig: ListenerConfig{
