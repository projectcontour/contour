...
func newSecret(secretType core_v1.SecretType, name string, namespace string, data map[string][]byte) *core_v1.Secret {
...
func WriteSecretsKube(client *kubernetes.Clientset, secrets []*core_v1.Secret, force OverwritePolicy) error {
	for _, s := range secrets {
		if _, err := client.CoreV1().Secrets(s.Namespace).Create(context.TODO(), s, meta_v1.CreateOptions{}); err != nil {
			if errors.IsAlreadyExists(err) && force == NoOverwrite {
...
func AsSecrets(namespace string, certdata *certs.Certificates) []*core_v1.Secret {
	return []*core_v1.Secret{
		newSecret(core_v1.SecretTypeTLS,
...
func AsLegacySecrets(namespace string, certdata *certs.Certificates) []*core_v1.Secret {
	return []*core_v1.Secret{
		newSecret(core_v1.SecretTypeTLS,
			"contourcert", namespace,
			map[string][]byte{
		...
		newSecret(core_v1.SecretTypeOpaque,
			"cacert", namespace,
			map[string][]byte{
