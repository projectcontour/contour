...
func MatchesIngressClassName(ing *networking_v1.Ingress, ingressClassName string) bool {
	annotationClass := annotation.IngressClass(ing)
	specClass := pointer.StringPtrDerefOr(ing.Spec.IngressClassName, "")

	// If annotation is set, check if it matches.
	if annotationClass != "" {
		return annotation.MatchesIngressClass(ing, ingressClassName)
	}
