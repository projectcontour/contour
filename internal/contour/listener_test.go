...
func TestListenerCacheQuery(t *testing.T) {
	tests := map[string]struct {
		...
		"fallback match": {
			contents: listenermap(&v2.Listener{
				Name:         ENVOY_FALLBACK_CERTIFICATE,
				Address:      envoy.SocketAddress("0.0.0.0", 8080),
				FilterChains: envoy.FilterChains(envoy.HTTPConnectionManager(ENVOY_FALLBACK_CERTIFICATE, envoy.FileAccessLogEnvoy(DEFAULT_HTTP_ACCESS_LOG), 0)),
			}),
			query: []string{ENVOY_FALLBACK_CERTIFICATE},
			want: []proto.Message{
				&v2.Listener{
					Name:         ENVOY_FALLBACK_CERTIFICATE,
					Address:      envoy.SocketAddress("0.0.0.0", 8080),
...
func TestListenerVisit(t *testing.T) {
		...
		"httpproxy with fallback certificate": {
			...
			want: listenermap(&v2.Listener{
				...
				FilterChains: []*envoy_api_v2_listener.FilterChain{{
					...
					Filters:         envoy.Filters(fallbackCertFilterFor()),
	...
	for name, tc := range tests {
		t.Run(name, func(t *testing.T) {
			...
			if tc.fallbackCertificate != nil {
				builder.FallbackCertificate = &k8s.FullName{
					Name:      tc.fallbackCertificate.Name,
					Namespace: tc.fallbackCertificate.Namespace,
				}
			}
