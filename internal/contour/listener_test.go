		...
		"--envoy-http-access-log": {
			...
			want: listenermap(&v2.Listener{
				...
				FilterChains: envoy.FilterChains(envoy.HTTPConnectionManager(ENVOY_HTTP_LISTENER, "/tmp/http_access.log",
					DEFAULT_RATE_LIMIT_SERVICE, DEFAULT_RATE_LIMIT_DOMAIN, DEFAULT_RATE_LIMIT_STAGE, DEFAULT_RATE_LIMIT_FAILURE_MODE_DENY)),
			}, &v2.Listener{
				Name:    ENVOY_HTTPS_LISTENER,
...
func defaultFilters() []*listener.Filter {
	return envoy.Filters(envoy.HTTPConnectionManager(
		ENVOY_HTTPS_LISTENER,
		DEFAULT_HTTPS_ACCESS_LOG,
		DEFAULT_RATE_LIMIT_SERVICE,
		DEFAULT_RATE_LIMIT_DOMAIN,
		DEFAULT_RATE_LIMIT_STAGE,
		DEFAULT_RATE_LIMIT_FAILURE_MODE_DENY))
}

func defaultFilterChains() []*listener.FilterChain {
	return envoy.FilterChains(envoy.HTTPConnectionManager(
		ENVOY_HTTP_LISTENER,
