	...
	for _, routeParentRef := range parentRefs {
		...
		for _, listener := range allowedListeners {
			...
			if routeKind != KindTCPRoute {
				...
				hosts, errs = p.computeHosts(routeHostnames, string(ref.Val(listener.listener.Hostname, "")), otherListenerHostnames)
...
//     defensive here.
func (p *GatewayAPIProcessor) computeHosts(routeHostnames []gatewayapi_v1beta1.Hostname, listenerHostname string, otherListenerHosts []string) (sets.Set[string], []error) {
