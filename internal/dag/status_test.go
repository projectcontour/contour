...
func TestDAGIngressRouteStatus(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"fallback certificate requested but cert not configured in contour": {
			objs: []interface{}{fallbackCertificate, fallbackSecret, sec1, s4},
			want: map[k8s.FullName]Status{
				{Name: fallbackCertificate.Name, Namespace: fallbackCertificate.Namespace}: {Object: fallbackCertificate, Status: "invalid", Description: "Spec.Virtualhost.TLS.FallbackCertificate requested but Certificate Secret not configured in Contour configuration file", Vhost: "example.com"},
			},
		},
		"fallback certificate requested and clientValidation also configured": {
			...
			want: map[k8s.FullName]Status{
				{Name: fallbackCertificateWithClientValidation.Name, Namespace: fallbackCertificateWithClientValidation.Namespace}: {Object: fallbackCertificateWithClientValidation, Status: "invalid", Description: "Spec.Virtualhost.TLS.FallbackCertificate & Spec.Virtualhost.TLS.ClientValidation are not compatible together", Vhost: "example.com"},
			},
		},
	}
