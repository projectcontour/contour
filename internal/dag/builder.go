	...
	if tls := proxy.Spec.VirtualHost.TLS; tls != nil {
		...
		if !tls.Passthrough {
			...
			if proxy.Spec.VirtualHost.TLS.EnableFallbackCertificate && tls.ClientValidation != nil {
				sw.SetInvalid("Spec.Virtualhost.TLS.FallbackCertificate & Spec.Virtualhost.TLS.ClientValidation are incompatible together")
				return
			}
			...
			if proxy.Spec.VirtualHost.TLS.EnableFallbackCertificate {
				if b.FallbackCertificate == nil {
					sw.SetInvalid("Spec.Virtualhost.TLS.FallbackCertificate requested but the fallback Certificate Secret is not configured in Contour configuration file")
					return
				}
