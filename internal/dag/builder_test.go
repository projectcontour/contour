	...
	tests := map[string]struct {
			...
			want: listeners(
				&Listener{
					...
					VirtualHosts: virtualhosts(
						virtualhost("test.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
					),
				},
			),
		},
		...
		"insert basic single route, single hostname, gateway From namespace selector": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("test.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardServiceCustomNs))),
					),
				},
			),
		},
		...
		"one gateway with two httproutes, different hostnames": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("another.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
						virtualhost("test.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
					),
				},
			),
		...
		"multiple hosts": {
			...
			want: listeners(
				&Listener{
					...
					VirtualHosts: virtualhosts(
						virtualhost("test.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
						virtualhost("test2.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
						virtualhost("test3.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
						virtualhost("test4.projectcontour.io", true, prefixrouteHTTPRoute("/", service(kuardService))),
					),
				},
			),
		},
		"no host defined": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("*", true, prefixrouteHTTPRoute("/", service(kuardService))),
					),
				},
			),
		},
		"wildcard hostname": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(virtualhost("*.projectcontour.io", true,
						&Route{
							PathMatchCondition: prefixString("/"),
							Clusters:           clustersWeight(service(kuardService)),
						}),
		...
		"missing service": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("*", true, directResponseRoute("/", http.StatusInternalServerError)),
					),
				},
			),
		},
		...
		"HTTPRoute references a backend in a different namespace, with valid ReferenceGrant": {
			...
			want: listeners(&Listener{
				Name:         "http-80",
				VirtualHosts: virtualhosts(virtualhost("*", true, prefixrouteHTTPRoute("/", service(kuardService)))),
			}),
		},
		...
		"HTTPRoute rule with request mirror filter": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(virtualhost("test.projectcontour.io", true,
		...
		"different weights for multiple forwardTos": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("*", true, prefixrouteHTTPRoute("/",
							&Service{
		...
		"weight of zero for a single forwardTo results in 500": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("*", true, directResponseRouteService("/", http.StatusInternalServerError, &Service{
							Weighted: WeightedService{
								Weight:           0,
								ServiceName:      kuardService.Name,
								ServiceNamespace: kuardService.Namespace,
		...
		"insert gateway listener with host": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("gateway.projectcontour.io", true,
							exactrouteHTTPRoute("/blog", service(kuardService))),
					),
				},
			),
		...
		"GRPCRoute: insert basic single grpc route, single hostname": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("test.projectcontour.io", true, exactrouteGRPCRoute("/io.projectcontour/Login", grpcService(kuardService, "h2c"))),
					),
				},
			),
		},
		...
		"GRPCRoute: references a backend in a different namespace, with valid ReferenceGrant": {
			...
			want: listeners(&Listener{
				Name:         "http-80",
				VirtualHosts: virtualhosts(virtualhost("*", true, exactrouteGRPCRoute("/io.projectcontour/Login", grpcService(kuardService, "h2c")))),
			}),
		},
	}
		...
		"ingressv1: insert ingress w/ default backend": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("*", true, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"ingressv1: insert ingress w/ host name and single backend": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("kuard.example.com", true, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"ingressv1: insert service, secret then ingress w/ tls": {
			...
			want: listeners(
				...
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("kuard.example.com", true, sec1, prefixroute("/", service(s1))),
					),
				},
			),
		},
		"ingressv1: insert service w/ secret with w/ blank ca.crt": {
			...
			want: listeners(
				...
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("kuard.example.com", true, sec3, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"ingressv1: insert ingress w/ two vhosts then matching service": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("a.example.com", true, prefixroute("/", service(s1))),
						virtualhost("b.example.com", true, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"ingressv1: insert ingress w/ two vhosts then service then secret": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("b.example.com", true, sec1, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"ingressv1: insert ingress w/ two paths then one service": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("b.example.com", true,
		...
		"ingressv1: insert ingress w/ two paths httpAllowed: false then tls and service": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("b.example.com", true, sec1,
							prefixroute("/", service(s1)),
							prefixroute("/kuarder", service(s2)),
						),
					),
		...
		"ingressv1: insert ingress w/ force-ssl-redirect: true": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("b.example.com", true, routeUpgrade("/", service(s1))),
					),
				}, &Listener{
					Name: HTTPS_LISTENER_NAME,
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("b.example.com", true, sec1, routeUpgrade("/", service(s1))),
					),
				},
			),
		},
		...
		"insert httpproxy with tls version 1.2": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("foo.com", false, routeUpgrade("/", service(s1))),
					),
				}, &Listener{
					Name: HTTPS_LISTENER_NAME,
					Port: 8443,
		...
		"insert httpproxy with invalid tls version": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("foo.com", false, sec1, routeUpgrade("/", service(s1))),
					),
				},
			),
		},
		...
		"insert httpproxy referencing two backends, one missing": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, prefixroute("/", service(s2))),
					),
				},
			),
		},
		...
		"insert httpproxy referencing two backends": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, prefixroute("/", service(s1), service(s2))),
					),
				},
			),
		},
		...
		"insert httpproxy w/ valid timeoutpolicy": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("bar.com", false, &Route{
							PathMatchCondition: prefixString("/"),
							Clusters:           clustermap(s1),
		...
		"ingressv1: insert ingress with wildcard hostnames": {
			...
			want: listeners(
				&Listener{
					...
					VirtualHosts: virtualhosts(
						virtualhost("*", true, prefixroute("/", service(s1))),
						virtualhost("*.example.com", true, &Route{
							PathMatchCondition: &PrefixMatchCondition{Prefix: "/"},
							HeaderMatchConditions: []HeaderMatchCondition{
								{
									Name:      ":authority",
		...
		"ingressv1: insert ingress overlay": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("example.com", true, sec13,
							routeUpgrade("/", service(s13a)),
							prefixroute("/.well-known/acme-challenge/gVJl5NWL2owUqZekjHkt_bo3OHYC2XNDURRRgLI5JTk", service(s13b)),
						),
					),
		...
		"insert httproxy": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"insert httproxy with invalid include": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, &Route{
		...
		"insert httpproxy with downstream verification": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, routeUpgrade("/", service(s1))),
					),
				}, &Listener{
					Name: HTTPS_LISTENER_NAME,
					Port: 8443,
		...
		"service shared across ingress and httpproxy tcpproxy": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", true, prefixroute("/", service(s9))),
					),
				},
				&Listener{
					Name: HTTPS_LISTENER_NAME,
			...
			want: listeners(
				&Listener{
					...
					VirtualHosts: virtualhosts(
						...
						virtualhost("example.com", false, prefixroute("/", service(s9))),
					),
				},
				&Listener{
					Name: HTTPS_LISTENER_NAME,
		...
		"HTTPProxy request redirect policy": {
			...
			want: listeners(
				&Listener{
					Name: HTTP_LISTENER_NAME,
					Port: 8080,
					VirtualHosts: virtualhosts(virtualhost("projectcontour.io", false,
						&Route{
							PathMatchCondition: prefixString("/"),
							Redirect: &Redirect{
								Scheme:     "https",
		...
		"ingressv1: Ingress then HTTPProxy with identical details, except referencing s2a": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", true, prefixroute("/", service(s2a))),
					),
				},
			),
		},
		...
		"ingressv1: insert service, secret, delegation, then ingress w/ tls and delegation annotation": {
			...
			want: listeners(
				...
				&Listener{
					...
					Port: 8443,
					SecureVirtualHosts: securevirtualhosts(
						securevirtualhost("kuard.example.com", true, sec4, prefixroute("/", service(s1))),
					),
				},
			),
		},
		...
		"httpproxy with fallback certificate enabled": {
			...
			want: listeners(
				&Listener{
					...
					Port: 8080,
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, routeUpgrade("/", service(s9))),
					),
				},
				&Listener{
					Name: HTTPS_LISTENER_NAME,
		...
		"httpproxy with fallback certificate enabled - another not enabled": {
			...
			want: listeners(
				&Listener{
					...
					VirtualHosts: virtualhosts(
						virtualhost("example.com", false, routeUpgrade("/", service(s9))),
						virtualhost("projectcontour.io", false, routeUpgrade("/", service(s9))),
					),
				},
				&Listener{
					Name: HTTPS_LISTENER_NAME,
		...
		"HTTPProxy attached to HTTP-only Gateway": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("kuard.projectcontour.io", false, prefixroute("/", service(kuardService))),
					),
				},
			),
		},
		...
		"HTTPProxy with TLS attached to Gateway with HTTP and HTTPS listener": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("kuard.projectcontour.io", false, routeUpgrade("/", service(kuardService))),
					),
				},
				&Listener{
					Name: "https-443",
					SecureVirtualHosts: []*SecureVirtualHost{
						securevirtualhost("kuard.projectcontour.io", false, sec1, routeUpgrade("/", service(kuardService))),
					},
				},
			),
		},
		...
		"Ingress attached to HTTP-only Gateway": {
			...
			want: listeners(
				&Listener{
					Name: "http-80",
					VirtualHosts: virtualhosts(
						virtualhost("kuard.projectcontour.io", true, prefixroute("/", service(kuardService))),
					),
				},
			),
		},
		...
		"Ingress with TLS attached to Gateway with HTTP and HTTPS listener": {
			...
			want: listeners(
				...
				&Listener{
					Name: "https-443",
					SecureVirtualHosts: []*SecureVirtualHost{
						securevirtualhost("kuard.projectcontour.io", true, sec1, prefixroute("/", service(kuardService))),
					},
				},
			),
		},
	...
	run(t, "root proxy with no route conditions refers to a missing service", testcase{
		...
		wantListeners: listeners(
			&Listener{
				...
				Port: 8080,
				VirtualHosts: virtualhosts(
					virtualhost("example.com", false, directResponseRoute("/", http.StatusServiceUnavailable)),
				),
			},
		),
		wantStatus: map[types.NamespacedName]contour_api_v1.DetailedCondition{
	...
	run(t, "root proxy with prefix route condition refers to a missing include", testcase{
		...
		wantListeners: listeners(
			&Listener{
				...
				Port: 8080,
				VirtualHosts: virtualhosts(
					virtualhost("example.com", false, directResponseRoute("/", http.StatusBadGateway)),
				),
			},
		),
		wantStatus: map[types.NamespacedName]contour_api_v1.DetailedCondition{
	...
	run(t, "root proxy includes child proxy that refers to a missing service", testcase{
		...
		wantListeners: listeners(
			&Listener{
				...
				Port: 8080,
				VirtualHosts: virtualhosts(
					virtualhost("example.com", false, directResponseRoute("/missing", http.StatusServiceUnavailable)),
				),
			},
		),
		wantStatus: map[types.NamespacedName]contour_api_v1.DetailedCondition{
...
func virtualhost(name string, shouldSort bool, first *Route, rest ...*Route) *VirtualHost {
	return &VirtualHost{
		Name:             name,
		Routes:           routes(append([]*Route{first}, rest...)...),
		ShouldSortRoutes: shouldSort,
	}
}

func securevirtualhost(name string, shouldSort bool, sec *v1.Secret, first *Route, rest ...*Route) *SecureVirtualHost {
